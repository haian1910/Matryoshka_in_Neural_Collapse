...
class BertWithMRLWrapper(nn.Module):
    def __init__(self, bert_model, nesting_list, num_labels, efficient=False):
        super(BertWithMRLWrapper, self).__init__()
        self.bert = bert_model
        self.mrl_classifier = MRL_Linear_Layer(
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None, labels=None, **kwargs):
        ...
        # Apply MRL classification layers
        logits_dict = {}
        if self.mrl_classifier.efficient:
            # Efficient mode: use only the largest nesting size
            full_embedding = pooled_output  # Should be of size nesting_list[-1]
            logits = self.mrl_classifier.nesting_classifier_0(full_embedding)
            logits_dict[f"logits_{self.nesting_list[-1]}"] = logits
        else:
            # Regular mode: apply classification to each nesting size
            for i, nesting_size in enumerate(self.nesting_list):
                # Truncate embedding to nesting size
                truncated_embedding = pooled_output[:, :nesting_size]
                classifier = getattr(self.mrl_classifier, f"nesting_classifier_{i}")
                logits = classifier(truncated_embedding)
                logits_dict[f"logits_{nesting_size}"] = logits
        
        # Return in format similar to transformers classification models
        if labels is not None:
            ...
            total_loss = total_loss / len(logits_dict)  # Average loss across all nesting sizes
...
class Distiller(nn.Module):
    ...
    def add_distiller_args(parser):
        ...
        # MRL specific arguments
        group.add_argument("--mrl-nesting-list", type=int, nargs='+', default=[64, 128, 256, 512, 768],
                           help='List of nesting dimensions for Matryoshka representation learning')
        group.add_argument("--mrl-efficient", action='store_true',
                           help='Use efficient MRL mode (only largest nesting size)')
        return parser
    ...
    def load_student_model(self):
        ...
        if self.args.peft is not None: #for Qwen3-Embedding
        ...
        else: #for BERT with MRL wrapper
            ...
            # Load base BERT model (not for sequence classification)
            ...
            max_nesting = max(nesting_list)
            if max_nesting > self.hidden_size:
                log_rank(f"Warning: Max nesting size {max_nesting} exceeds hidden size {self.hidden_size}. "
                        f"Adjusting nesting list.")
                nesting_list = [min(size, self.hidden_size) for size in nesting_list]
            
            # Wrap BERT with MRL classifier
            model = BertWithMRLWrapper(
                bert_model=bert_model,
                nesting_list=nesting_list,
                num_labels=self.args.num_labels,
                efficient=mrl_efficient
            )
            
            log_rank(' > number of parameters: {:,}'.format(
                sum([p.nelement() for p in model.parameters()])
            ))
            log_rank(f' > MRL nesting list: {nesting_list}')
            log_rank(f' > MRL efficient mode: {mrl_efficient}')
